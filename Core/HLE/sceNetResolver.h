...
private:
    static std::shared_ptr<SceNetResolver> g_instance;

    int m_currentNetResolverId = 1;
    std::unordered_map<u32, std::shared_ptr<NetResolver>> m_netResolvers;
    std::shared_mutex m_netResolversMutex;
};
