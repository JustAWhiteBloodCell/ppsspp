...
int NetResolver_StartNtoA(u32 resolverId, u32 hostnamePtr, u32 inAddrPtr, int timeout, int retry) {
	...
	if (!sceNetResolver) {
		return hleLogError(SCENET, ERROR_NET_RESOLVER_STOPPED, "Net Resolver Subsystem Shutdown: Resolver %i", resolverId);
	}
...
static int sceNetResolverStartNtoAAsync(int resolverId, u32 hostnamePtr, u32 inAddrPtr, int timeout, int retry) {
	ERROR_LOG_REPORT_ONCE(sceNetResolverStartNtoAAsync, SCENET, "UNIMPL %s(%d, %08x, %08x, %d, %d) at %08x", __FUNCTION__, resolverId, hostnamePtr, inAddrPtr, timeout, retry, currentMIPS->pc);
...
static int sceNetResolverPollAsync(int resolverId, u32 unknown) {
	ERROR_LOG_REPORT_ONCE(sceNetResolverPollAsync, SCENET, "UNIMPL %s(%d, %08x) at %08x", __FUNCTION__, resolverId, unknown, currentMIPS->pc);
...
static int sceNetResolverWaitAsync(int resolverId, u32 unknown) {
	ERROR_LOG_REPORT_ONCE(sceNetResolverWaitAsync, SCENET, "UNIMPL %s(%d, %08x) at %08x", __FUNCTION__, resolverId, unknown, currentMIPS->pc);
...
static int sceNetResolverStartAtoN(int resolverId, u32 inAddr, u32 hostnamePtr, int hostnameLength, int timeout, int retry) {
	ERROR_LOG_REPORT_ONCE(sceNetResolverStartAtoN, SCENET, "UNIMPL %s(%d, %08x[%s], %08x, %i, %i, %i) at %08x", __FUNCTION__, resolverId, inAddr, ip2str(*(in_addr*)&inAddr, false).c_str(), hostnamePtr, hostnameLength, timeout, retry, currentMIPS->pc);
...
static int sceNetResolverStartAtoNAsync(int resolverId, u32 inAddr, u32 hostnamePtr, int hostnameLength, int timeout, int retry) {
	ERROR_LOG_REPORT_ONCE(sceNetResolverStartAtoNAsync, SCENET, "UNIMPL %s(%d, %08x[%s], %08x, %i, %i, %i) at %08x", __FUNCTION__, resolverId, inAddr, ip2str(*(in_addr*)&inAddr, false).c_str(), hostnamePtr, hostnameLength, timeout, retry, currentMIPS->pc);
	return 0;
}

static int sceNetResolverCreate(u32 resolverIdPtr, u32 bufferPtr, int bufferLen) {
	WARN_LOG(SCENET, "UNTESTED %s(%08x[%d], %08x, %d) at %08x", __FUNCTION__, resolverIdPtr, Memory::Read_U32(resolverIdPtr), bufferPtr, bufferLen, currentMIPS->pc);
...
static int sceNetResolverStop(u32 resolverId) {
	auto sceNetResolver = SceNetResolver::Get();
	if (!sceNetResolver) {
		return hleLogError(SCENET, ERROR_NET_RESOLVER_STOPPED, "Resolver Subsystem Stopped (Resolver Id: %i)", resolverId);
	}
...
std::shared_ptr<SceNetResolver> SceNetResolver::g_instance;

void SceNetResolver::Init() {
	g_instance = std::make_shared<SceNetResolver>();
}
...
std::shared_ptr<NetResolver> SceNetResolver::GetNetResolver(u32 resolverId) {
	std::shared_lock lock(m_netResolversMutex);
	const auto it = m_netResolvers.find(resolverId);
	return it != m_netResolvers.end() ? it->second : nullptr;
}
...
std::shared_ptr<NetResolver> SceNetResolver::CreateNetResolver(u32 bufferPtr, u32 bufferLen) {
	...
	std::unique_lock lock(m_netResolversMutex);
	int currentNetResolverId = m_currentNetResolverId++;
	return m_netResolvers[currentNetResolverId] = std::make_shared<NetResolver>(
...
bool SceNetResolver::DeleteNetResolver(u32 resolverId) {
	std::unique_lock lock(m_netResolversMutex);
	const auto it = m_netResolvers.find(resolverId);
	if (it == m_netResolvers.end())
		return false;
	m_netResolvers.erase(it);
	return true;
}

void SceNetResolver::ClearNetResolvers() {
	std::unique_lock lock(m_netResolversMutex);
	for (auto& it : m_netResolvers) {
		it.second->SetIsRunning(false);
	}
	m_netResolvers.clear();
}
